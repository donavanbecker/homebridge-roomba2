...
import { RoombaPlatformConfig } from './settings.js'
...
async function iRobotLogin(body: any, server: number = 1): Promise<any> {
    return new Promise((resolve, reject) => {
        ...
        const req = https.request(iRobotLoginOptions, (res) => {
            ...
            res.on('end', () => {
                try {
                    iRobotLoginResponse(null, res, JSON.parse(data), resolve, reject, body, server)
                } catch (e: any) {
                    if (server === 1) {
                        iRobotLogin(body, 2).then(resolve).catch(reject)
                    } else {
                        iRobotLoginResponse(e.message ?? e, undefined, undefined, resolve, reject, body, server)
                    }
                }
            })
        })
        ...
        req.on('error', (error) => {
            iRobotLoginResponse(error, undefined, undefined, resolve, reject, body, server)
        })
...
function iRobotLoginResponse(error: Error | null, _response?: IncomingMessage, body?: any, resolve?: (value: any) => void, reject?: (reason?: any) => void, credentials?: any, server?: number): void {
    if (error) {
